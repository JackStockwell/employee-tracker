// Imports

// const mysql = require('mysql2');
const inquirer = require('inquirer');
const { mainMenu, addDepartment, addRole, addEmployee, updateEmployee } = require('./questions');
const { EmployeeDB, Departments, Employees, Roles } = require('../db/employeeDatabase.js');
const { response } = require('express');

class CLI extends EmployeeDB {

    constructor(db) {
        super(db)
    }

    // Renders parsed data into a table.
    render(data) {
        console.log('\n')
        console.table(data)
        console.log('\n')
    }

    viewDepartments = async () => {
        const depList = await this.retrieveDepartments();
        this.render(depList);
        this.mainMenu();
    }

    viewRoles = async () => {
        const roleList = await this.retrieveRoles();
        this.render(roleList);
        this.mainMenu();
    }

    viewEmployees = async () => {
        const empList = await this.retrieveEmployees();
        this.render(empList);
        this.mainMenu();
    }

    promptAddDepartments() {
        // Inquirer prompt
        inquirer
            .prompt(addDepartment)
            .then((response) => {
                // Takes the department response name, inserts it into the database.
                this.addDepartment(response)
                // Logs the the result, 
                .then((result) => {
                    console.log('\x1b[32m%s\x1b[0m', result)
                    this.mainMenu();
                })
            })
            .catch((err) => {
                throw new Error(err)
            })
    }

    promptAddRoles = async () => {
        // Retrieves a the list of departments.
        const list = await this.retrieveDepartments();
        
        // Dynamicall retrieves a list from the database to render as choice.
        // Loops through the list
        list.forEach((department) => {
            console.log(department)
            // Creates a choice object to be used by inquirer
            const choice = {value: department.id, name: department.name}
            // Pushed into the choices array to be presented.
            addRole[2].choices.push(choice)
        })
        // Inquirer prompt
        inquirer
            .prompt(addRole)
            // Takes the values recieved by inquirer.
            .then((response) => {
                console.log(response)
                // Object is parsed into the addRole func, which inserts it into the database.
                this.addRole(response)
                    .then((result) => {
                        console.log('\x1b[32m%s\x1b[0m', result)
                        this.mainMenu()
                    });
            })
    }

    promptAddEmployees = async () => {

        // Retrieves the current roles from the Database.
        const roleList = await this.retrieveRoles();
        // Goes through all the roles, adds them to the choices in the addEmployee roles inquirer prompt and pushes them in.
        roleList.forEach((role) => {
            const choice = {value: role.id, name: role.title};
            addEmployee[2].choices.push(choice)
        })
        // Retrieves the current employees from the Database.
        const empList = await this.retrieveEmployees();
        // Goes through all the employees, adds them to the choices in the addEmployee employees inquirer prompt and pushes them in.
        empList.forEach((employee) => {
            const choice = {value: employee.id, name:employee.Name};
            addEmployee[3].choices.push(choice)
        });

        inquirer
            .prompt(addEmployee)
            .then((response) => {
                this.addEmployee(response);

            })

    }

    promptUpdateEmployee = async () => {

        // Retrieves the current employees from the Database.
        const empList = await this.retrieveEmployees();
        // Goes through all the employees, adds them to the choices in the inquirer prompt
        empList.forEach((employee) => {
            const choice = {value: employee.id, name:employee.Name};
            updateEmployee[0].choices.push(choice)
        });

        // Retrieves the current roles from the Database.
        const roleList = await this.retrieveRoles();
        // Goes through all the roles, adds them to the choices in the inquirer prompt
        roleList.forEach((role) => {
            const choice = {value: role.id, name: role.title};
            updateEmployee[1].choices.push(choice)
        })

        inquirer
            .prompt(updateEmployee)
            .then((response) => {
                console.log(response)
                // this.updateEmployeeRole(response)
            })
    }

    mainMenu() {
        inquirer
        .prompt(mainMenu)
        .then((response) => {
            console.log(response.option)

            switch(response.option) {
                case 'viewDepartments':
                    this.viewDepartments();
                    break;
                case 'viewRoles':
                    this.viewRoles();
                    break;
                case 'viewEmployees':
                    this.viewEmployees();
                    break;
                case 'addDepartment':
                    this.promptAddDepartments();
                    break;
                case 'addRole':
                    this.promptAddRoles();
                    break;
                case 'addEmployee':
                    this.promptAddEmployees();
                    break;
                case 'updateRole':
                    this.promptUpdateEmployee();
                    break;
                
            }
        })
    }
 
}

module.exports = CLI;